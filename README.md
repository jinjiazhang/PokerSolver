...
> **🔥 最新优化 (209MB 巨型完美哈希表平铺):** 从源头开启**空间换时间**，将 $O(n)$ 的牌力计算降维为 $O(1)$ 数组直接查表寻址。大幅消除了原有的 hash_map 跳跃及分支分歧，尤其在深层发牌博弈中（如 MCCFR）实现了近十倍吞吐暴增。

| 场景 | 范围大小 | 迭代次数 | 每次迭代 | 用时 |
|------|----------|----------|----------|------|
| River | 1,081 × 1,081 | 50 | **0.4ms** | 0.02s |
| Turn | 20 × 36 | 50 | **0.9ms** | 0.04s |
| Turn (全范围) | 1,128 × 1,128 | 10 | **395ms** | 4.00s |
| Flop (全树展开) | 32 × 32 | 10 | **245ms** | 2.50s |
| Flop (**MCCFR 采样**) | 32 × 32 | 2000 | **0.2 ~ 1.3ms** | **2.6s** |

> Flop 测试环境：Board AhKdQc, OOP范围 32 组合, IP范围 32 组合, 8 线程并行

### 累计优化加速比

| 场景 | 初始版本 | 前置版本 | 💎 最新大内存版 (209MB) | 加速比 |
|------|---------|---------|-----------------------|--------|
| Turn 20×36 | 297 ms/iter | 1.8 ms/iter | **0.9 ms/iter** | **330×** |
| River 1081×1081 | 117 ms/iter | 0.5 ms/iter | **0.4 ms/iter** | **292×** |
| Flop MCCFR | - | 4.1 ms/iter | **0.2 ~ 1.3 ms/iter** | 🚀 **大跃升** |
...
```json
{
  ...
  "root_strategy": {
    ...
    "hands": {
      "KcKh": {
        "strategy": [0.0000, 0.0000, 0.0000, 1.0000],
        "ev": 150.0000,
        "ev_action": [95.5000, 110.0000, 120.0000, 150.0000]
      },
      "AcAd": {
        "strategy": [0.0000, 0.0000, 0.0000, 1.0000],
        "ev": 150.0000,
